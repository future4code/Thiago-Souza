...
import {
  ...
  removerProfessor as removerProfessorDatabase
} from "../../database/mysql";
...
professorRouter.delete("/:id", removerProfessor);
professorRouter.delete("/:id/turma", removerDaTurma);
...
async function removerDaTurma(request: Request, response: Response): Promise<void> {
  ...
  try {
    const professor = await removerProfessorDaTurma(id);
    if (!professor) {
      response.status(404).send(erros.professorNaoEcontrada);
      return;
    }

    response.send("Professor removido da turma");
  } catch (erro) {
    response.status(500).send(erros.inesperado);
  }
}

async function removerProfessor(request: Request, response: Response)
: Promise<void> {
  const { id } = request.params;
  if (!validarUUID(id)) {
    response.status(400).send(erros.IDInvalido);
  ...
  try {
    const professor = await removerProfessorDatabase(id);
    if (!professor) {
      response.status(404).send(erros.professorNaoEcontrada);
      return;
    }

    response.send("Professor removido");
  } catch (erro) {
    response.status(500).send(erros.inesperado);
  }
}
